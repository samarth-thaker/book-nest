...
class _LendingTabState extends State<LendingTab>
  ...
  // --- CustomBookTile confirmation state ---
  Book? _justLentBook;
  ...
  void _showLentBookTile(Book book) async {
    setState(() {
      _justLentBook = book;
    });
  ...
  void _lendBook() {
    ...
    // Show the confirmation tile
    _showLentBookTile(updatedBook);
    ...
    return Scaffold(
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_justLentBook != null)
              FadeTransition(
                opacity: _tileFadeAnimation,
                child: GestureDetector(
                  onTap: () {
                    _tileAnimController.reverse().then((_) {
                      if (mounted) setState(() => _justLentBook = null);
                    });
                  },
                  child: CustomBookTile(
                    book: _justLentBook!,
                    showStatus: true, // Pass any custom props you want
                    // You can add a close button, etc.
                  ),
                ),
              ),
            if (_justLentBook != null) const SizedBox(height: 12),
...
class _LendingTabState extends State<LendingTab>
  ...
  void _showLentBookTile(Book book) async {
    ...
    // Removed the auto-hide functionality - tile will now persist
  }
...
Widget build(BuildContext context) {
  ...
  // Sort: Overdue books first, then by most recent lent date
  lentBooks.sort((a, b) {
    if (a.isOverdue && !b.isOverdue) return -1;
    if (!a.isOverdue && b.isOverdue) return 1;
    if (a.lentDate != null && b.lentDate != null) {
