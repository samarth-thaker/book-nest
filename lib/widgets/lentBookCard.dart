...
class LentBookCard extends StatelessWidget {
  final Book book;
  
  const LentBookCard({Key? key, required this.book}) : super(key: key);
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text('Lent to: ${book.lentToPersonName}'),
              ],
            ),
            
            // Lent date
            if (book.lentDate != null)
              Row(
                children: [
                  const Icon(Icons.calendar_today, size: 16),
                  const SizedBox(width: 4),
                  Text('Lent on: ${_formatDate(book.lentDate!)}'),
                ],
              ),
  ...
  void _sendReminder(BuildContext context, Book book) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Reminder sent to ${book.lentToPersonName}'),
      ),
    );
  }
  ...
  void _markAsReturned(BuildContext context, Book book) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Mark "${book.title}" as returned by ${book.lentToPersonName}?'),
        actions: [
          TextButton(
